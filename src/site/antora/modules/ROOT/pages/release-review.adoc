...
The scripts need PowerShell 7.4 or newer and refuse to run on anything older, because the setting
that makes a failing `gpg` abort the script exists only from 7.4.
Windows PowerShell 5.1, the `powershell.exe` that ships with Windows, is not enough; use `pwsh`.
