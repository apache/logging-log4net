...
Note that %property expands context values, which may carry user-supplied data, so a path built from one is only as trustworthy as that value, see the https://logging.apache.org/security/faq.html#path-traversal[security FAQ on path traversal].
...
A configured value whose name reads as a secret, such as `password` or `connectionString`, is
replaced with `*****` in these messages, so the property is still named but the value is not shown.
