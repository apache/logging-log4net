...
|How long a logging call waits for room in a full queue, in milliseconds. Defaults to `5000`.
`0` never waits and discards the mail instead.
...
* The mail is handed to a background thread rather than sent by the thread that logged, so a slow
server no longer stalls logging. Failures are reported to the error handler as before, but after
