...
It writes log events in batches (with a default size of 512, controlled by the `BufferSize` setting).
...
[WARNING]
====
Always configure `CommandText` together with `parameter` elements, as every example below does.

If `CommandText` is omitted, the appender falls back to a legacy mode in which the rendered
`Layout` output *is* the SQL statement that gets executed.
Layouts perform no SQL quoting or escaping and offer no way to add it, so anything that reaches
a log statement (a user name, a request parameter, an exception message) is executed as part
